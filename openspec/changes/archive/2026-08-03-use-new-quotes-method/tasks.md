...
- [x] 5.3 Run sanitized Yandex compatibility checks on supported Obsidian desktop and mobile runtimes with package 0.0.3, then append only version, status, and aggregate structural/count evidence to `tests/fixtures/yandex/runtime-observations.json`; do not record tokens, book metadata, CSV content, highlights, or comments.
